import os, uuid, threading, time, shutil, subprocess, tempfile
...
def ffmpeg_upscale(in_path, out_path, scale=2):
    cmd = ['ffmpeg', '-y', '-i', in_path, '-vf', f'scale=iw*{scale}:ih*{scale}:flags=lanczos', '-c:v', 'libx264', '-preset', 'fast', '-crf', '22', out_path]
...
def try_realesrgan_python(in_path, out_path, scale=2):
    ...
    # Very simple and slow: extract frames, upscale frames, reassemble
    ...
    try:
        ...
        for f in frames:
            ...
            out.save(p)
        subprocess.run(['ffmpeg','-y','-framerate','25','-i', os.path.join(tmp,'frame_%06d.png'), '-c:v','libx264','-pix_fmt','yuv420p', out_path], check=True)
    finally:
        shutil.rmtree(tmp)
...
def upscale_worker(job_id, in_path, out_path):
    ...
    try:
        # Try python realesrgan first (may not be installed); if fails, fallback to ffmpeg scaling
        try:
            try_realesrgan_python(in_path, out_path, scale=2)
        except Exception:
            # fallback to ffmpeg
            # simulate progress while ffmpeg runs
